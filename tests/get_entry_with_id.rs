...
async fn returns_status_ok_if_exists(_pool: SqlitePool) {
    let test_database =
        "get_entry_with_id/returns_status_ok_if_exists";
    let client =
...
#[sqlx::test(fixtures("entries"))]
async fn returns_entry_ok_response_if_exists(_pool: SqlitePool) {
    let test_database =
        "get_entry_with_id/returns_entry_ok_response_if_exists";
    let client =
        client::get_http_client_with_database(test_database).await;
    let test_uri = format!("{}/{}", TEST_URI, &EXISTING_ENTRY_ID);
    let uri = Origin::parse(&test_uri).expect("URI to be valid");
    let response = client.get(uri).dispatch().await;
    let body = response.into_json::<OkResponse<Entry>>().await;
    assert!(body.is_some());
}
...
async fn returns_status_404_if_not_exists(_pool: SqlitePool) {
    let test_database =
        "get_entry_with_id/returns_status_404_if_not_exists";
    let client =
        client::get_http_client_with_database(test_database).await;
...
async fn returns_string_error_response_if_not_exists(
    ...
    let test_database = "get_entry_with_id/\
...
async fn returns_status_500_when_error(_pool: SqlitePool) {
    let test_database =
        "get_entry_with_id/returns_status_500_when_error";
    let client =
        client::get_http_client_with_database(test_database).await;
    let test_uri = format!("{}/{}", TEST_URI, &EXISTING_ENTRY_ID);
    let uri = Origin::parse(&test_uri).expect("URI to be valid");
...
async fn returns_string_error_response_when_error(_pool: SqlitePool) {
    let test_database =
        "get_entry_with_id/returns_string_error_response_when_error";
    let client =
        client::get_http_client_with_database(test_database).await;
    let test_uri = format!("{}/{}", TEST_URI, &EXISTING_ENTRY_ID);
    let uri = Origin::parse(&test_uri).expect("URI to be valid");
